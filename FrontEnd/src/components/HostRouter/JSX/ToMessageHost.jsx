...
// import {noteContextSo} from './NoteState/NoteState'
const ToMessage = () => {
  const [insideMessage,setInsidMessage]=useState([]);
  const {TOSendToMEssage,PersonalFetchAllMEssage}=useSend();
          ...
          <div className="message-container">
            {insideMessage.length>=1 && insideMessage[0]?.messages?.length > 0 ? insideMessage.map((item,index)=>{
              return(
                ...
                const isMine = msg.from === "host";
                return (
                  <div className={`msg-row ${isMine ? "mine" : "theirs"}`} key={i}>
                    <span className={`msg-tag ${isMine ? "you" : "User"}`}>
                      {isMine ? "You" : "User"}
                    </span>
